                ...
                <div class="inner">
                   <form id="ContentListingForm" accept-charset="utf-8" action="<?php echo Yii::app()->request->baseUrl; ?>/admin/contentsort" method="post" name="ContentSortForm">
                    <div id="content-admin-input" class="container-inline">    
                        <label>title
                            <input type="text" value="<?php echo $TitleSort; ?>" id="Content_title" name="Content[title]">
                        </label>
                    </div>
                        ...
                        <label>type is
                            <select name="Content[type]" id="select-type">
                                ...
                                if(!empty(Yii::app()->session['type'])):
                                    foreach($Type as $t): 
                                ?>
                                    <option <?php if($t->id == Yii::app()->session['type']): ?> selected="selected" <?php endif; ?> value="<?php echo $t->id; ?>"><?php echo $t->name; ?></option>
                                <?php 
                                    endforeach;
                                else: 
                                ?>
					...
					<div id="content-admin-input" class="container-inline">    
                        <label>Create Time
                            <?php if(!empty(Yii::app()->session['create_time'])): ?>
                                <input type="text" name="Content[create_time]" class="datepicker" value="<?php echo Yii::app()->session['create_time']; ?>" >
                            <?php else: ?>
                    ...
                    <div id="content-admin-input" class="container-inline">    
                        <label>status is
                            <select name="Content[publish]" id="select-type">
                                <?php 
                                if(Yii::app()->session['publish'] == '1'){
                                    echo '<option value="1">publish</option>';
                                    echo '<option value="0">unpublish</option>';
                                } elseif (Yii::app()->session['publish'] == '0') {
                                    echo '<option value="0">unpublish</option>';
                                    echo '<option value="1">publish</option>';
                                } elseif (empty(Yii::app()->session['publish'])) {
                                    echo '<option value="1">publish</option>';
                                    echo '<option value="0">unpublish</option>';
                                }
                                ?>
