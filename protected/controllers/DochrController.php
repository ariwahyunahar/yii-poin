...
class elFinder {
	...
	public function run() {
		...
		$ech = json_encode($this->_result);
		// log cmd
		Yii::app()->db->createCommand()->insert('uss_doc_hr_logs', array(
		    'nik' => Yii::app()->user->username,
...
class DochrController extends Controller
{
	...
	public function actionIndex()
	{
		...
		// cek hak akses
		if(count(UssDocHr::anggotaDOkHr(Yii::app()->user->username))<1){
			$this->redirect(Yii::app()->request->baseUrl . '/');
		}
		
		$q = new CSqlDataProvider("select distinct x.nik, y.name from uss_doc_hr x 
			left join member y on x.nik = y.nik where x.nik not in ('101344') order by y.name
		");
		$peserta_doc_khss = $q->getData();
		
		$peserta_doc_khssA2 = Yii::app()->db->createCommand('select distinct x.nik, y.name from uss_doc_hr x 
	...
	public function actionConnectordochr() {
		 ...
		 **/
		$opts = array(
			'root'          => $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'assets/dochr/',  // path to root directory
			'URL'           => 'assets/dochr/', // root directory URL
			'rootAlias'     => 'Documents HCM',       // display this instead of root directory name
