...
class DocgmController extends Controller
{
	...
	{
		if (Yii::app()->user->isGuest) $this->redirect(Yii::app()->request->baseUrl . '/site/login');
		
		if(count(UssDocKhusus::anggotaDOkKhusus(Yii::app()->user->username))<1){
			$this->redirect(Yii::app()->request->baseUrl . '/');
		}
		
		$peserta_doc_khss = Yii::app()->db->createCommand("select distinct x.nik, y.name from uss_doc_khusus x 
			left join member y on x.nik = y.nik where x.nik not in ('101344', '030710') and 1=0 order by y.name")->queryAll();
		// $qA = new CSqlDataProvider("select distinct x.nik, y.name from uss_doc_khusus x 
			// left join member y on x.nik = y.nik order by y.name
		// ");
		// $peserta_doc_khssA = $qA->getData();
		
		$peserta_doc_khssA = Yii::app()->db->createCommand('select distinct x.nik, y.name from uss_doc_khusus x 
			left join member y on x.nik = y.nik order by y.name')->queryAll();
				
		// echo "<pre>";print_r($peserta_doc_khss);die();
		...
		//$this->globalVars['activeMenu']['docbrowser'] = 'active';
		
		$vars = array(
			'peserta_doc_khss'=>$peserta_doc_khss
		, 'peserta_doc_khssA'=>$peserta_doc_khssA
		);
	...
	public function actionConnectorm() {
		...
		$opts = array(
			'root'          => $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.Yii::app()->params['docBrowsergm']['root'],  // path to root directory
			'URL'           => Yii::app()->params['docBrowsergm']['url'], // root directory URL
			'rootAlias'     => 'Documents',       // display this instead of root directory name
	...
	public function actionConnector() {
		...
		$opts = array(
			'root'            => $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.Yii::app()->params['docBrowser']['root'],  // path to root directory
			'URL'             => Yii::app()->params['docBrowser']['url'], // root directory URL
			'rootAlias'       => 'Documents',       // display this instead of root directory name
			'uploadAllow'   => array('all'),
			'uploadDeny'    => array(),
			'uploadOrder'   => 'allow,deny',
